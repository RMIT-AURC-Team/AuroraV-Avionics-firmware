...
void vIdle(void *argument) {
  ...
  for (;;) {
    ...
    if (*ctx->currentState >= LAUNCH && ctx->mem.pageReady)
      xEventGroupSetBits(ctx->xTaskEnableGroup, GROUP_TASK_ENABLE_FLASH);
  }
}
...
void vFlashBuffer(void *argument) {
  ...
  uint8_t outBuff[ctx->flash.pageSize];

  for (;;) {
    // Wait for write flag to be ready, clear flag on exit
    EventBits_t uxBits = xEventGroupWaitBits(ctx->xTaskEnableGroup, GROUP_TASK_ENABLE_FLASH, pdTRUE, pdFALSE, timeout);
    if (uxBits & GROUP_TASK_ENABLE_FLASH) {
      bool success = ctx->mem.readPage(&ctx->mem, outBuff); // Flush data to output buffer
      if (success) {
        // Write data to flash memory
        ctx->flash.writePage(&ctx->flash, pageAddr, outBuff);
        pageAddr += 0x100;
      }
    }
  }
